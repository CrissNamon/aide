...
public enum ArgumentMatcherHolder {
...

  {
     ...
     */
    addMatcher(
        Factory.INVOKE, (holder, original, args) -> fromVoid(() -> holder.getWrapper().invoke()));
    addMatcher(
        Factory.ACTION,
    ...
    addMatcher(
        Factory.CONSUMER_ARGS_2,
        (holder, original, args) -> fromVoid(
            () -> holder.getWrapper().accept(args[0], args[1], args[2]))
    );
    addMatcher(
        Factory.CONSUMER_ARGS_3,
        (holder, original, args) -> fromVoid(
            () -> holder.getWrapper().accept(args[0], args[1], args[2],
                                             args[3]
            ))
    ...
    addMatcher(
        Factory.FUNCTION_ARGS_1,
        (holder, original, args) -> holder.getWrapper().apply(args[0], args[1])
    );
    addMatcher(
        Factory.FUNCTION_ARGS_2,
        (holder, original, args) -> holder.getWrapper().apply(args[0], args[1],
                                                              args[2]
        )
    );
    addMatcher(
        Factory.FUNCTION_ARGS_3,
        (holder, original, args) -> holder.getWrapper().apply(args[0], args[1],
                                                              args[2], args[3]
        )
    );
    addMatcher(
        Factory.FUNCTION_ARGS_4,
        (holder, original, args) -> holder.getWrapper().apply(args[0], args[1],
                                                              args[2], args[3],
                                                              args[4]
        )
  ...
  public <T> T apply(WrapperHolder holder, Executable original, Object[] args) {
    ...
    MatcherSignature matcherSignature = new MatcherSignature(
        holder.getDeclaringInterface(), signature);
    BooleanOptional.of(argumentMatchers.containsKey(matcherSignature))
        .ifFalseThrow(() -> ReflectionException.format(
                          "No matchers found for %s!"
                              + "See ArgumentMatcherHolder#addMatcher",
                          original
                      )
        );
