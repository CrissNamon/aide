...
public class IfTrueConditionalTest {
  ...
  public void branchTest() {
    ...
    Mockito.when(booleanPredicate.test(any())).thenReturn(false);
    Mockito.when(stringSupplier.get()).thenReturn("result");

    String actual = (String) IfTrueConditional.create().ifTrue(false).then("false")
                                                       .ifTrue(null, booleanPredicate)
                                                       .then(stringSupplier).ifTrue(true)
                                                       .then(stringSupplier).orElse("WRONG");
  ...
  public void orElseTest() {
    ...
    String actual = (String) IfTrueConditional.create().ifTrue(false).then("false")
                                                       .ifTrue(null, booleanPredicate)
                                                       .then(stringSupplier).ifTrue(false)
                                                       .then(stringSupplier).orElse("WRONG");
